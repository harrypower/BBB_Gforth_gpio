...
\ This Gforth talkes to a DTH11 or DTH22 sensor on BBB rev c hardware 
...
\ i think i got the bellow stuff done for at least a test
...
: config-dth-type
    ...
    s" -22_" search
    if
	22 to dth-11-22? get_pin if get_temp_humd bye else ." Header & pin not allowed with DTH22 sensor!" cr bye then
    then
    s" -11_" search
    if
	11 to dth-11-22? get_pin if get_temp_humd bye else ." Header pin not allowed with DTH11 sensor!" cr bye then
    then
